...
{#if image}
	...
	>
		<div
			class="neo-box-lg max-w-4xl max-h-[92vh] w-full bg-surface p-4 flex flex-col space-y-3 animate-slide-down"
			role="dialog"
			aria-modal="true"
		...
		>
			...
			<div class="flex items-center justify-between border-b-2 border-border-color pb-2.5 font-mono">
				<div class="truncate pr-3">
					<h4 class="text-sm font-bold text-text-primary uppercase truncate">{image.title}</h4>
				...
				<div class="flex items-center gap-2 shrink-0">
					<button
						...
						class="neo-btn text-xs py-1 px-3"
